...
const GENERIC_SYNC_ERROR_CODE = 'sync_internal_error';
...
export function describeSyncError(err: unknown): { message: string; code: string } {
  const prismaCode = err && typeof err === 'object' && 'code' in err ? (err as { code?: unknown }).code : undefined;

  if (prismaCode === 'P2003') {
    return { message: FOREIGN_ENTITY_ERROR, code: FOREIGN_ENTITY_ERROR_CODE };
  }

  if (err !== undefined) {
    console.error('[sync] Fehler beim Anwenden eines Sync-Events:', err);
  }
  return { message: GENERIC_SYNC_ERROR_MESSAGE, code: GENERIC_SYNC_ERROR_CODE };
}
