...
- **`Athlete.notes`** (freies Coaching-Notizfeld): wird für Rolle `athlete` grundsätzlich redigiert (leerer String) — sowohl beim Pull als auch in der `serverVersion` einer Konfliktantwort — unabhängig davon, ob es der eigene oder ein fremder Athletendatensatz ist. Push-Versuche, die `notes` ändern wollen, werden auf den bisherigen Serverstand zurückgesetzt, ohne den Rest des Updates zu blockieren.
  Auch der Selbstbedienungs-Export (`GET /api/me/export`) liefert `notes` nicht aus — bewusste Vereinsentscheidung (Issue #94): Auskunft nach Art. 15 DSGVO zu diesen Notizen erteilt der Verein auf Anfrage. Beim Löschen des Athletenprofils (Löschanfrage des Kontos oder Sync-`delete`) werden die Notizen sofort geleert, nicht erst beim Hard-Purge; das Löschen eines Trainer:innen-Kontos lässt sie unberührt (Notizen sind Vereinsdaten zur Athletin/zum Athleten, ohne Autor:innen-Zuordnung).
...
- ✅ **Abhängigkeits-Audit, inzwischen automatisiert in der CI-Pipeline** (nicht im ursprünglichen Plan vorgesehen — ursprünglich nur ein einmaliger manueller Lauf, siehe Abschnitt 12): `nodemailer` wies mehrere bekannte High-Severity-CVEs auf (SMTP-/CRLF-Injection u. a.) und wurde von `6.x` auf `9.0.3` angehoben (Changelog auf Kompatibilität mit dem hier genutzten Codepfad geprüft); die transitive Abhängigkeit `fast-uri` wurde per `npm audit fix` gepatcht. `.github/workflows/ci.yml` blockiert seitdem den Merge automatisch bei künftigen High-/Critical-Funden in Produktionsabhängigkeiten.
...
- ✅ **Migrationen — jetzt tatsächlich umgesetzt (Code-Review, Befund W5):** `apps/api/prisma/migrations/` ist nicht mehr per `.gitignore` ausgeschlossen und enthält eine committete, reviewbare Migrationshistorie (erste Migration `20260824173611_init`, per `prisma migrate dev --name init` gegen eine echte Postgres-Instanz erzeugt und verifiziert — Ergebnis stimmt mit `schema.prisma` überein, `prisma migrate diff` zwischen beiden liefert eine leere Migration). CI (`.github/workflows/ci.yml`) und jedes Deployment (`docs/deployment*.md`, `scripts/setup-codespace.sh`, `.devcontainer/devcontainer.json`) wenden sie nicht-interaktiv per `prisma migrate deploy` an — vorher lief überall `prisma db push` (erzeugt das Schema direkt aus `schema.prisma`, ohne Historie, ohne Review-Möglichkeit für einzelne Änderungen, ohne Rollback-Pfad; siehe der vorherige Stand dieses Eintrags, damals aus Sandbox-Gründen ohne Internetzugriff auf `binaries.prisma.sh` unvermeidbar). Eine künftige Schemaänderung entsteht lokal per `npm run prisma:migrate -- --name <kurze-beschreibung>` und wird committet.
- **Hosting:** Statt der ursprünglich verglichenen verwalteten Anbieter (Fly.io/Railway) wurde eine **eigene vServer-Lösung (Hetzner) mit Docker** gewählt und in `docs/deployment/deployment.md` dokumentiert — Begründung: geringere laufende Kosten für einen einzelnen Verein, Wartungsaufwand als vertretbar bewertet.
- **Neu, nicht im ursprünglichen Plan:** Der Purge-Cron-Job (`npm run purge-deleted-data`, Abschnitt 14) muss zusätzlich zum eigentlichen App-Deployment eingerichtet werden — Details in `docs/deployment/deployment.md` und in Abschnitt 14 unten.
...
- **`npm audit` ist jetzt Teil der CI-Pipeline** (nachträglich ergänzt, siehe `.github/workflows/ci.yml`): ein blockierender Schritt `npm audit --omit=dev --audit-level=high` (schlägt den PR-Check fehl, sobald eine Produktionsabhängigkeit eine High- oder Critical-Schwachstelle aufweist — genau die Schwelle, die den `nodemailer`-Fund aus Abschnitt 15 erkannt hätte) sowie ein zweiter, nur informativer Schritt ohne `--omit=dev` (`continue-on-error: true`) für Dev-Abhängigkeiten wie die bekannte esbuild/vite-Kette (siehe unten). Damit ist dieser Punkt **nicht mehr offen** — er verbleibt hier nur als Verweis auf den Anlass.
...
| — | `npm audit` war nicht Teil der CI-Pipeline — der `nodemailer`-Fund kam nur durch einen manuellen Lauf ans Licht | `.github/workflows/ci.yml` | Blockierender `npm audit --omit=dev --audit-level=high`-Schritt (Produktionsabhängigkeiten) plus ein informativer, nicht-blockierender Lauf für Dev-Abhängigkeiten |
...
**Migration — erledigt (Code-Review, Befund W5):** `schema.prisma` wurde aktualisiert (`comments`-Spalten auf `Exercise`/`Plan`); die inzwischen committete erste Migration (`20260824173611_init`, siehe Abschnitt 10) legt beide Spalten korrekt an, da sie vom zu diesem Zeitpunkt bereits aktuellen `schema.prisma`-Stand ausgeht. Eine bereits produktiv migrierte Bestands-Datenbank (auf einem Stand vor `20260824173611_init`) bräuchte weiterhin einen eigenen, nachträglichen Migrationsschritt für nur diese beiden Spalten — betrifft dieses Repository nicht, da es vor dieser ersten Migration noch kein Produktivsystem gab.
...
CI-Pipeline (`.github/workflows/ci.yml`): Typecheck (`tsconfig.typecheck.json`, inkl. `test/`/`scripts/`/`prisma/`) → Lint → Test → Build, für alle Workspaces. **Nachträglich ergänzt** (siehe Abschnitt 12): ein blockierender `npm audit --omit=dev --audit-level=high`-Schritt direkt nach der Installation, plus ein informativer, nicht-blockierender Audit-Lauf für Dev-Abhängigkeiten.
